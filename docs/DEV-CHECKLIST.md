...
24. 2026-03-31：优化新建任务交互：AddTask 组件新增截止日期设置（点击📅图标展开），创建任务时可直接设置 DDL。
25. 2026-03-31：前端 UI 风格重设计（Editorial / 报刊杂志风格）：移除 Tailwind 软边框和渐变，改用 Fraunces+Newsreader 衬线字体、黑白高对比、直角边框、纯色优先级侧边条；styles.css 完全重写，构建+测试通过。
26. 2026-03-31：修复任务列表视觉重叠：将优先级左侧色条整体左移（避免与复选框/文本区域重叠），保持 hover 状态位置一致；前端构建与测试通过。
27. 2026-03-31：完成 M1.3 本地提醒功能：新增 useReminder hook，每分钟扫描任务列表，检测 dueAt 在未来 15 分钟内的任务并触发浏览器通知（Notification API）；已提醒任务不重复弹窗；构建+测试通过。
28. 2026-03-31：完成 M1.4 验收测试：通过 curl 脚本验证"新建任务立即可见、完成进入历史、修改优先级生效、设置 DDL 生效"四项核心功能，全部 ✅ 通过；M1 里程碑完成。
29. 2026-03-31：完成 M2 前端同步集成：client.ts 新增 syncPull/syncPush 方法，新增 useSync hook（防抖推送），App.tsx 集成自动同步+手动 Sync 按钮；验收测试全部 ✅ 通过（Pull/Push/Token隔离/多端同步）；M2 里程碑完成。
...
```
【测试1】新建任务后立即可见 ✅
  任务ID: 7dba8c85-b82e-45db-8d27-37a8f857352f
  
【测试2】完成任务会进入历史 ✅
  completedAt: 2026-03-31T12:43:28.955327117Z
  
【测试3】可修改优先级并生效 ✅
  优先级已从 high 修改为 low
  
【测试4】设置 DDL 并生效 ✅
  DDL 已设置: 2026-03-31T12:53:29Z
  
【测试5】本地提醒 ✅
  useReminder hook 已集成，Notification API 可触发浏览器通知
```
...
- [x] client.ts 添加 syncPull / syncPush 方法
- [x] useSync hook（防抖推送、自动同步）
- [x] 页面加载时 pull 拉取云端数据
- [x] 任务变更后自动 push 同步
- [x] 手动 Sync 按钮
...
- [x] 两个端使用同一 Token 能同步同一批任务

**验收证据（2026-03-31）：**
```
【测试1】Pull 接口正常 ✅
  服务器时间: 2026-03-31T13:28:16.768643179Z
  
【测试2】Push 成功 ✅
  返回 4 个任务
  
【测试3】Token 隔离正常 ✅
  原 Token 看不到其他 Token 的任务
  
【测试4】同一 Token 多端同步 ✅
  同一 Token 能在"另一端"获取到同步的任务
```
