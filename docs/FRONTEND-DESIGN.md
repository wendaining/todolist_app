...
├── hooks/
│   └── useReminder.ts     # 本地提醒 Hook（DDL 到期通知）
├── App.tsx                # 主组件（含主题切换、提醒）
├── App.test.tsx           # App 测试
├── main.tsx               # 入口
...
### 7.1 实现方案
- **Hook**：`useReminder(tasks)` 自定义 Hook
- **扫描间隔**：每 60 秒扫描一次任务列表
- **提醒窗口**：dueAt 在未来 15 分钟内触发通知
- **通知 API**：`Notification.requestPermission()` + `new Notification(...)`
- **去重机制**：`notifiedIds` Set 记录已提醒任务，避免重复弹窗
- **内存清理**：任务删除后自动清理对应的提醒记录
...
export function useReminder(tasks: Task[]) {
  // 每分钟扫描，检测 dueAt 在窗口内的任务
  // 触发 Notification，记录已提醒 ID
}
